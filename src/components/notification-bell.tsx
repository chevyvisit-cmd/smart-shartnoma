...
import { acceptSentContract, rejectContract, getPendingContracts } from "@/lib/actions";
            ...
            >
              ...
              <div className="flex items-center justify-between border-b border-white/5 px-4 py-3">
                <div className="flex items-center gap-2">
                  {viewState !== "list" && (
                    <button onClick={backToList} className="flex h-7 w-7 items-center justify-center rounded-lg text-muted-foreground hover:text-foreground transition-colors">
                      <ChevronLeft size={16} />
                    </button>
                  )}
                ...
                <button onClick={() => setOpen(false)} className="flex h-7 w-7 items-center justify-center rounded-lg text-muted-foreground hover:text-foreground transition-colors">
                  <X size={14} />
                </button>
              </div>
              ...
              <AnimatePresence mode="wait">
                ...
                {/* LIST */}
                {viewState === "list" && (
                  <motion.div key="list" initial={{ opacity: 0, x: -16 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -16 }} transition={{ duration: 0.15 }} className="max-h-[460px] overflow-y-auto">
                      ...
                      <div key={contract.id} className="border-b border-white/5 px-4 py-4 last:border-0">
                        <div className="mb-3 flex items-start gap-3">
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="text-sm font-black truncate">{contract.title}</p>
                            <p className="text-xs text-muted-foreground mt-0.5">
                              {uz ? "Yuboruvchi: " : "Отправитель: "}
                              <span className="font-bold">{contract.creator.name || contract.creator.phone}</span>
                            </p>
                            {contract.amount ? (
                              <p className="text-xs font-bold text-primary mt-0.5">{contract.amount.toLocaleString()} UZS</p>
                            ) : null}
                          </div>
                        </div>
                        <div className="flex items-center gap-2">
                ...
                {viewState === "terms" && activeContract && (
                  <motion.div key="terms" initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 16 }} transition={{ duration: 0.15 }} className="flex max-h-[500px] flex-col">
                    <div className="border-b border-white/5 px-4 py-3">
                      ...
                      {activeContract.amount ? (
                        <p className="text-xs font-bold text-primary mt-1">{activeContract.amount.toLocaleString()} UZS</p>
                      ) : null}
                    </div>
                      ...
                      {parseTerms(activeContract.terms).length > 0 ? (
                        <>
                          ...
                          {parseTerms(activeContract.terms).map((term, i) => (
                            <button key={i} onClick={() => toggleTerm(i)} className="flex w-full items-start gap-3 rounded-xl border border-white/5 bg-white/5 px-3 py-3 text-left transition-all hover:bg-white/10">
                              <span className="mt-0.5 shrink-0 text-primary">
                                {checkedTerms[i] ? <CheckSquare size={16} /> : <Square size={16} className="text-muted-foreground" />}
                              </span>
                              <span className={`text-xs leading-relaxed ${checkedTerms[i] ? "text-foreground font-bold" : "text-muted-foreground"}`}>
                                <span className="mr-1 font-black text-primary/60">{i + 1}.</span>{term}
                              </span>
                            </button>
                          ))}
                        </>
                    ...
                    <div className="border-t border-white/5 px-4 py-3 space-y-2">
                      ...
                      >
                        <Check size={16} />
                        {loading ? (uz ? "Saqlanmoqda..." : "Сохраняется...") : (uz ? "Tasdiqlash va qabul qilish" : "Подтвердить и принять")}
                      </button>
                    </div>
                  </motion.div>
                )}
                ...
                {viewState === "reject" && activeContract && (
                  <motion.div key="reject" initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 16 }} transition={{ duration: 0.15 }} className="px-4 py-4 space-y-3">
                    <p className="text-sm font-black">{activeContract.title}</p>
                    ...
                    <div className="flex gap-2">
                      <button onClick={handleReject} disabled={loading} className="flex-1 rounded-xl bg-red-500/10 py-2.5 text-xs font-black text-red-500 hover:bg-red-500/20 transition-colors disabled:opacity-50">
                        {loading ? t.rejecting : t.rejectConfirm}
                      </button>
                      <button onClick={backToList} className="flex-1 rounded-xl bg-white/5 py-2.5 text-xs font-black text-muted-foreground hover:bg-white/10 transition-colors">
                        {t.rejectCancel}
                      </button>
                    </div>
                  </motion.div>
